...
# —————————————————————————————
# Константы
SRC_SS_ID       = "14GdnqsE0Z0lbNF99vby553gwdeUzMMdPVrVXI9byuCA"
SRC_SHEET_TITLE = "refunds brazil"

DST_SS_ID       = "1HItT2-PtZWoldYKL210hCQOLg3rh6U1Qj6NWkBjDjzk"
DST_SHEET_TITLE = "refunds data"
# —————————————————————————————
...
def main():
    ...
    df = df_all.iloc[:, [0, 1, 2, 3, 4, 5, 6, 7]]
    logging.info(f"→ Отобрано {len(df)} строк с колонками")
    
    # 4) Пишем в целевой лист
    sh_dst = client.open_by_key(DST_SS_ID)
    ws_dst = sh_dst.worksheet(DST_SHEET_TITLE)
    ws_dst.batch_clear(['A:H'])
